...
if($header['Authorization']){
    ...
    $id = (int)$token['ref'];
    	if($id && filter_has_var(INPUT_POST, 'id') && filter_has_var(INPUT_POST, 'interest')){
    		try{
    			require_once '..'.DIRECTORY_SEPARATOR.'Defy'.DIRECTORY_SEPARATOR.'store.php';
                ...
                Pos_Follower::$_db=$database;
                    $details = Pos_Follower::getBiodata($_POST['id'],$_POST['interest'],$id);   
        ...
        // else{
        //     echo json_encode(array('state'=>false, 'error'=>'Incomplete parameters'));
        // }
}
?>
    	...
    	if($id && filter_has_var(INPUT_POST, 'current')){
    		try{
                ...
                if($_POST['current'] == 'country'){
                	$details = Pos_Follower::getCountryPeople($id,$_POST['interest'],$_POST['country']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }elseif($_POST['current'] == 'state'){
                	$details = Pos_Follower::getStatePeople($id,$_POST['interest'],$_POST['country'],$_POST['state']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }elseif ($_POST['current'] == 'local') {
                	$details = Pos_Follower::getLocalPeople($id,$_POST['interest'],$_POST['country'],$_POST['state'],$_POST['local']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }      
            }catch(Exception $e){

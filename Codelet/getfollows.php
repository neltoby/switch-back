    	...
    	if($id && filter_has_var(INPUT_POST, 'current')){
    		try{
                ...
                if($_POST['current'] == 'country'){
                	$details = Pos_Follower::getCountryFollower($id,$_POST['interest'],$_POST['country']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }elseif($_POST['current'] == 'state'){
                	$details = Pos_Follower::getStateFollower($id,$_POST['interest'],$_POST['country'],$_POST['state']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }elseif ($_POST['current'] == 'local') {
                	$details= Pos_Follower::getLocalFollower($id,$_POST['interest'],$_POST['country'],$_POST['state'],$_POST['local']);
                    $details['token'] = $token['token'];  
                    echo json_encode($details); 
                }      
            }catch(Exception $e){
